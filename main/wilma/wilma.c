...
static esp_err_t update_ssid_list(void)
{
	...
	esp_blufi_ap_record_t * blufi_ap_list = (esp_blufi_ap_record_t *)malloc(WILMA_VISIBLE_APS_COUNT * sizeof(esp_blufi_ap_record_t));
	if (!blufi_ap_list) {
		BLUFI_ERROR("malloc error, blufi_ap_list is NULL");
	} else {
		for (int i = 0; i < WILMA_VISIBLE_APS_COUNT; ++i)
		{
			blufi_ap_list[i].rssi = records[i].rssi;
			memcpy(blufi_ap_list[i].ssid, records[i].ssid, sizeof(records[i].ssid));
		}

		if (ble_is_connected == true) {
			esp_blufi_send_wifi_list(WILMA_VISIBLE_APS_COUNT, blufi_ap_list);
		} else {
...
static void wifi_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
	...
	if (event_base == WIFI_EVENT) {
		...
		case IP_EVENT_STA_GOT_IP: {
			...
			// Update the "state" to indicate authentication failure
			update_ssid_state(gl_sta_ssid, STA_STATE_OK);
...
static void wilma_thread(void *data)
{
	...
	ESP_ERROR_CHECK(esp_event_handler_instance_register(WIFI_EVENT, ESP_EVENT_ANY_ID, wifi_event_handler, NULL, &WILMA_INSTANCE_ANY_ID));
	ESP_ERROR_CHECK(esp_event_handler_instance_register(IP_EVENT, ESP_EVENT_ANY_ID, wifi_event_handler, NULL, &WILMA_INSTANCE_GOT_IP));
